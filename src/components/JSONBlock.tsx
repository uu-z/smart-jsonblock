...
const resolveComponent = (
  ...
  if (
    ...
    const requestedType = data._type as ComponentType;
    return componentMap[requestedType] || componentMap.generic;
  }
...
const JSONBlock: React.FC<JSONBlockProps> = ({
  ...
  const entries = Object.entries(data).filter(
    ([key]) => key !== "_type" && key !== "_layout" && key !== "_columns"
  );

  // 检查数据中是否指定了布局
  const dataLayout = (data as BaseJSONData)._layout || layout;
  const dataColumns = (data as BaseJSONData)._columns || columns;
    ...
    >
      {entries.map(([key, value]) => {
        ...
        if (
          ...
          componentType = value._type as ComponentType;
        } else {
          // 尝试查找匹配的模式
          for (const { type, matcher } of patternMatchers) {
            if (matcher(value)) {
        ...
        const wrapperStyles: React.CSSProperties = {
          gridColumn:
            dataLayout === "grid" && span > 1
              ? `span ${Math.min(span, dataColumns)}`
              : undefined,
        };
        ...
        return (
          <div
            key={key}
            className={`component-wrapper component-type-${
