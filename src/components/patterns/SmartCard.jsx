...
const SmartCard = ({ data }) => {
  ...
  // 自动检测卡片类型
  const detectedVariant = image ? 'media' : 
                 (icon && typeof content === 'string') ? 'info' :
                 (content && typeof content === 'object' && 'value' in content) ? 'stat' :
                 (Array.isArray(actions) && actions.length) ? 'action' : 'basic';
  
  // 使用显式指定的变体或检测到的变体
  const variant = explicitVariant || detectedVariant;
  
  console.log('SmartCard variant:', variant, 'data:', data);
  ...
  // 使用直接的样式属性处理自定义渐变
  const style = gradient && typeof gradient === 'string' ? { background: gradient, color: 'white' } : {};

  // 根据变体渲染不同的卡片内容
  const renderCardByVariant = () => {
    switch(variant) {
      case 'media':
        return (
          <>
      ...
      case 'stat':
        return (
          <div className="card-body text-center">
            ...
            {title && <h3 className="card-title">{title}</h3>}
            {subtitle && <p className="card-description">{subtitle}</p>}
            {content && content.value && <div className="card-value">{content.value}</div>}
            {content && content.description && <p className="card-description">{content.description}</p>}
            {renderActions(actions)}
            {footer && <div className="card-footer">{footer}</div>}
          </div>
        );
...
function renderContent(content) {
  ...
  if (typeof content === 'object') {
    ...
    if ('items' in content && Array.isArray(content.items)) {
      return (
        <ul className="card-list">
          {content.items.map((item, i) => <li key={i}>{typeof item === 'string' ? item : JSON.stringify(item)}</li>)}
        </ul>
      );
    }
